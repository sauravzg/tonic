...
pub trait PushStreamConsumer {
    /// The type of item consumed.
    type Item: Send;
...
pub struct PushStreamWriter<T, C> {
    inner: C,
    _marker: PhantomData<fn(T) -> ()>,
}

impl<T, C> PushStreamWriter<T, C>
where
    T: Send,
    C: PushStreamConsumer<Item = T>,
{
    ...
    /// Writes an item to the stream.
    pub async fn write(&mut self, item: T) -> Result<(), Status> {
