...
pub trait PushStreamExt<T>: Sized {
    type ThenStream<U, F>;
...
impl<T, P> PushStreamExt<T> for PushStream<T, P>
where
    T: Send,
    P: PushStreamProducer<Item = T>,
{
    type ThenStream<U, F> = PushStream<U, ThenProducer<P, T, U, F>>;
...
pub struct ThenProducer<P, T, U, F> {
    inner: P,
    f: F,
    _phantom: PhantomData<fn(T) -> U>,
}

impl<P, T, U, F, Fut> PushStreamProducer for ThenProducer<P, T, U, F>
where
    P: PushStreamProducer<Item = T>,
    T: Send,
    U: Send,
